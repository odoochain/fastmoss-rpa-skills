"""Aggregate FastMoss creator CSVs into crosstabs for the analysis report.

Reads the CSVs produced by creator_scraper.py / creator_filtered.py and
prints cross-tabulations: country distribution, creator-category distribution,
ranking x country crosstab, top creators (multi-ranking presence).

Usage:
    python analyze.py --fans <path> [--commerce <path>] [--horse <path>] \\
...
def top_creators(all_rows, min_count=2):
    """Creators appearing in multiple rankings or multiple times in top-N."""
    seen = Counter()
    for r in all_rows:
...
def render_plain(stats):
    ...
    for section, data in stats.items():
        ...
        if isinstance(data, dict) and "rows" in data:
            ...
            # defaultdict(crosstab) — render top-3 per row
            for k, cnt in data.items():
                out.append(f"  {k}: {dict(cnt.most_common(3)) if hasattr(cnt, 'most_common') else cnt}")
...
def render_markdown(stats, out_path):
    ...
    cat_md = "\n".join(f"| {n} | {c} |" for n, c in cat_rows)
    rc_rows = []
    ...
    rc_md = "\n".join(rc_rows)
    repeat_md = "\n".join(f"- `{n}` × {c}" for n, c in stats["repeat_creators"]) or "_(无 — 各榜单均无重复达人)_"
...
def main():
    ...
    p.add_argument("--fans", help="涨粉达人榜 CSV (creator_scraper or creator_filtered output)")
    p.add_argument("--commerce", help="带货达人榜 CSV")
    p.add_argument("--blue-v", help="蓝V达人榜 CSV")
    p.add_argument("--popular", help="热门达人榜 CSV")
    p.add_argument("--horse", help="黑马达人榜 CSV")
    p.add_argument("--filtered", help="Combined multi-filter CSV (rank field used to split)")
    p.add_argument("--out-md", help="If given, write the full Markdown report here using the recipe template")
    args = p.parse_args()

    per_ranking_files = {
        "fans": args.fans,
        "commerceTop": args.commerce,
        "blue-v": args.blue_v,
        "popular": args.popular,
        "potentialTop": args.horse,
    }
    per_ranking = {k: read_csv(v) for k, v in per_ranking_files.items() if v}
    if args.filtered:
        # split a combined CSV by 'ranking' column
    ...
    stats = {
        ...
        "category_breakdown": breakdown(all_rows, "creator_category"),
        "ranking_country": cross_tab(all_rows, "ranking", "country"),
        "repeat_creators": top_creators(all_rows, min_count=2),
    }
