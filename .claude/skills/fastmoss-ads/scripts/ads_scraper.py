...
Usage:
    python ads_scraper.py --ranking tag --pages 5 --out <path>/tags.csv
    python ads_scraper.py --ranking keyword --pages 5 --out <path>/keywords.csv
    python ads_scraper.py --ranking category --pages 3 --out <path>/categories.csv
    python ads_scraper.py --url <custom-url> --pages 3 --out <path>/custom.csv
    python ads_scraper.py --help
"""
import argparse
import csv
import json
...
def main():
    ...
    p.add_argument("--url", help="Custom FastMoss ads trend URL (overrides --ranking)")
    ...
    nav = call("navigate", {"url": url, "newTab": True, "group_title": "fastmoss-ads"}, args.session)
