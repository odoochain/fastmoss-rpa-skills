"""FastMoss shop ranking scraper with country filter applied.
...
from shop_scraper import call, evaluate, scrape_current, sanitize_header, RANKINGS  # noqa: E402
...
def scrape_one(ranking_key, country_label, pages, session, nav_sleep, page_sleep):
    ...
    for p in range(pages):
        ...
        if p == 0 and rows:
            cats = {r.get("shop_category", "") for r in rows}
            print(f"[{ranking_key}/{country_label} page {p+1}] rows={len(rows)} categories={cats}")
        else:
...
def write_csv(path, rows):
    base_fields = ["page", "ranking", "filter_country",
                   "rank", "shop_name", "shop_legal_name", "shop_category", "shop_rating"]
    extra_fields = []
...
def main():
    ...
    p.add_argument("--ranking", required=True, choices=list(RANKINGS.keys()),
                   help="Shop ranking to scrape (one of: %s)" % ", ".join(RANKINGS))
