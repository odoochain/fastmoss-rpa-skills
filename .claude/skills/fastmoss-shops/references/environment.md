...
- If running multiple FastMoss sister skills in parallel, each one defaults to its own session name (`fastmoss-products`, `fastmoss-creators`, `fastmoss-shops`, etc.) — no conflict.
...
curl -s -X POST http://127.0.0.1:10086/command \
  ...
  -d '{"action":"navigate","args":{"url":"https://example.com","newTab":true},"session":"fastmoss-shops"}'
```
...
curl -s -X POST http://127.0.0.1:10086/command \
  -H 'Content-Type: application/json' \
  -d '{"action":"screenshot","args":{"format":"jpeg","quality":75},"session":"fastmoss-shops"}' \
