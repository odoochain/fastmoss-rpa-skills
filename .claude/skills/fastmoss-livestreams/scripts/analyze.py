"""Aggregate FastMoss livestream CSVs into crosstabs for the analysis report.

Reads the CSVs produced by live_scraper.py / live_filtered.py and
prints cross-tabulations: ranking distribution, country distribution,
ranking x country crosstab, top entities (multi-ranking presence).

Usage:
    python analyze.py --tiktok <path> [--hot-product <path>] \\
        [--live-commerce <path>] [--filtered <path>] [--out-md <path>]
"""
import argparse
import csv
...
def main():
    ...
    p.add_argument("--tiktok", help="TT直播榜 CSV")
    p.add_argument("--hot-product", help="直播爆品榜 CSV")
    p.add_argument("--live-commerce", help="直播带货达人榜 CSV")
