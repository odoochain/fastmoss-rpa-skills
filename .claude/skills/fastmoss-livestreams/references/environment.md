...
- If running multiple FastMoss sister skills in parallel, each one defaults to its own session name (`fastmoss-products`, `fastmoss-livestreams`, etc.) — no conflict.

## Bash + JSON escaping rules

### `bsk` CLI commands: works
```bash
bsk navigate --session fastmoss-livestreams "https://example.com"
bsk evaluate --session fastmoss-livestreams "(() => JSON.stringify({title: document.title}))()"
```
...
```bash
bsk screenshot --session fastmoss-livestreams | python -c "import sys,json,base64; open('out.jpeg','wb').write(base64.b64decode(json.load(sys.stdin)['data']['data']))"
```
...
- Default session `"fastmoss-livestreams"` for all calls (isolated from sister skills' sessions like `fastmoss-products`, `fastmoss-creators`). Override via `--session` if needed.
- `bsk session stop fastmoss-livestreams` closes ALL tabs in the session — always run at task end.
- `bsk tab create --session fastmoss-livestreams <url>` works whether or not the session pre-exists.
