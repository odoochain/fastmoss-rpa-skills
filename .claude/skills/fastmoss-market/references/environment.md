...
- If running multiple FastMoss sister skills in parallel, each one defaults to its own session name (`fastmoss-products`, `fastmoss-market`, etc.) — no conflict.

## Bash + JSON escaping rules

### `bsk` CLI commands: works
```bash
bsk navigate --session fastmoss-market "https://example.com"
bsk evaluate --session fastmoss-market "(() => JSON.stringify({title: document.title}))()"
```
...
```bash
bsk screenshot --session fastmoss-market | python -c "import sys,json,base64; open('out.jpeg','wb').write(base64.b64decode(json.load(sys.stdin)['data']['data']))"
```
...
- Default session `"fastmoss-market"` for all calls (isolated from sister skills' sessions like `fastmoss-products`, `fastmoss-creators`). Override via `--session` if needed.
- `bsk session stop fastmoss-market` closes ALL tabs in the session — always run at task end.
- `bsk tab create --session fastmoss-market <url>` works whether or not the session pre-exists.
